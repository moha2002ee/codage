...
namespace planning
{
	...
	{
		if(mode == READ){throw XmlFileSerializerException("Erreur; mode doit etre WRITE et pas READ ", XmlFileSerializerException::NOT_ALLOWED);}
		file << val << endl;
	...
	T XmlFileSerializer<T>::read()
	{
		...
		if (mode == WRITE){throw XmlFileSerializerException("Erreur; mode doit etre READ et pas WRITE", XmlFileSerializerException::NOT_ALLOWED);}
		streampos positionActuelle = file.tellg();

		file >> ligne;
		if(ligne == "</"+collectionName+">"){throw XmlFileSerializerException("fin du fichier atteinte", XmlFileSerializerException::END_OF_FILE);}
