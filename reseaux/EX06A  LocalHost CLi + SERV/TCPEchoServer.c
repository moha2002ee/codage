...
int main(int argc, char *argv[])
{
    ...
    int sService ;                    /* Socket descriptor for client Service */
    ...
    for (;;) /* Run forever */
    {
        ...
        if (( sService = accept(servSock, (struct sockaddr *) &echoClntAddr, 
                               &clntLen)) < 0)
        ...
        /* sService is connected to a client! */

        // printf("Handling client %s\n", inet_ntoa(echoClntAddr.sin_addr));
        // Recuperation d'information sur le client connecte
        {
	struct sockaddr_in adrClient; socklen_t adrClientLen = sizeof(struct sockaddr_in); // nécessaire
