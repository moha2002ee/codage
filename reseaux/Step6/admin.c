...
void AfficheEnteteVehiculeHV ()
{
 ...
 sprintf(Tampon,"%s","Quantite ") ; MonPrintf(Tampon,10,strlen(Tampon)) ;
...
void AfficheVehiculeHV (struct VehiculeHV    *UnRecord)
{
 ...
 sprintf(Tampon,"%d",UnRecord->Quantite ) ; MonPrintf(Tampon,10,strlen(Tampon)) ;
...
void SaiSieVehiculeHV (int Reference, struct VehiculeHV  *UnRecord )
{
 ...
 printf("choix couleur{ 1 :Jaune, 2:Vert, 3:Bleu, 4:Noir }:");
 fgets(Tampon,sizeof Tampon,stdin );

   switch(atoi(Tampon)){
   case 1:  
         strcpy(UnRecord -> Couleur,"jaune");
         break;
   case 2: 
         strcpy(UnRecord -> Couleur,"Vert");
         break;

   case 3: 
         strcpy(UnRecord -> Couleur,"Bleu");
         break;

   case 4: 
         strcpy(UnRecord -> Couleur,"Noir");
         break;
   default:
         printf("Choix invalide, auncune couleur assignee\n");
         strcpy(UnRecord->Couleur, "Inconnu");
         break;
...
void ListingVehiculesHV (char *NomFichier)
{
 ...
 while ( bytesRead )
 {
  fprintf(stderr,"Record lu %ld Bytes \n",lseek(fd, 0, SEEK_CUR)) ;
  AfficheVehiculeHV ( &UnRecord) ;
  bytesRead =read (fd, &UnRecord,sizeof(UnRecord));
 }
 close(fd) ;
